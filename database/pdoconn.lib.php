...
class pdoconn {
   ...
   var $saveSql, $excludeTables, $replicateSql, $repServer;
    ...
    protected function SaveSqlQuery($sql=null) {
	    ...
	    //check for table excludes
		foreach ($this->excludeTables as $t=>$table) {	
			if (stripos($sql, "$table "))
				return false;
		}
		...
		if ((stristr($sql, 'insert ')) || (stristr($sql, 'update ')) || (stristr($sql, 'delete '))) {
			...
			$sql = "INSERT INTO syncsql (fid,date,execdate,status,reference,sqlquery) VALUES (?,?,?,?,?,?)";//'$sqlquery','$reference',$status)";
			$this->db->prepare($sql)->execute([1,$now,$now,1,'system',$node['sqlquery']);			
    ...
    public function Connect($host,$user,$password,$name) {
		...
		if (($this->model=='PDO') && (defined(_PDODB_))) {
			try 
			{
		...
		elseif (($this->model=='ADODB') && (defined(_ADODB_))) { 
