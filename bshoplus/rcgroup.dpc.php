...
$__DPCSEC['RCGROUP_DPC']='1;1;1;1;1;1;2;2;9;9;9';

if ( (!defined("RCGROUP_DPC")) && (seclevel('RCGROUP_DPC',decode(GetSessionParam('UserSecID')))) ) {
define("RCGROUP_DPC",true);

$__DPC['RCGROUP_DPC'] = 'rcgroup';


$__EVENTS['RCGROUP_DPC'][0]='cpgroup';
$__EVENTS['RCGROUP_DPC'][1]='cpsavegrp';
$__EVENTS['RCGROUP_DPC'][2]='cploadgrp';
$__EVENTS['RCGROUP_DPC'][3]='cpsavexmlgrp';

$__ACTIONS['RCGROUP_DPC'][0]='cpgroup';
$__ACTIONS['RCGROUP_DPC'][1]='cpsavegrp';
$__ACTIONS['RCGROUP_DPC'][2]='cploadgrp';
$__ACTIONS['RCGROUP_DPC'][3]='cpsavexmlgrp';

$__LOCALE['RCGROUP_DPC'][0]='RCGROUP_DPC;Group items;Επιλογή ειδών';
$__LOCALE['RCGROUP_DPC'][1]='_GROUPITEMS;Group items;Επιλογή ειδών';
...
class rcgroup {
    ...
    function __construct() {
		...
		$ip = remote_paramload('RCGROUP','imagepath',$this->prpath);
		$ipath = $ip ? $ip : '/images/';
		$ia = remote_paramload('RCGROUP','imageabs',$this->prpath);
		if (!$ia) {
			$pt = remote_paramload('RCITEMS','phototype',$this->prpath);	
			$csize = remote_paramload('RCGROUP','itemphotosize',$this->prpath);
    ...
    function event($event=null) {
       ...
       if (!$this->msg) {
	     ...
	     switch ($event) {
            ...
            case 'cpsavexmlgrp'   : $this->savedXMLlist = $this->save_xml_file(); 
			                        break;				 
			 
		    case 'cploadgrp'      : $this->savedlist = $this->loadList(); 
			                        break;			 
		 
		    case 'cpsavegrp'      : $this->savedlist = $this->saveList();				
	                                break;									
			case 'cpgroup'        :
			default               :							  
         }
    ...
    function action($action=null)  { 
	     ...
	     switch ($action) {
           case 'cpsavexmlgrp'   : break;		 
		   
		   case 'cploadgrp'      : break;		   
		   
		   case 'cpsavegrp'      : $out = 'Filename:' . $this->filename . '<br/>';
...
*/	
	...
	protected function saveList() {
		...
		elseif ($this->savedXMLlist) {
			...
			if (!empty($_POST[$this->listName])) { //mygroup post array selector
	...
	protected function show_select_presets($name, $taction=null, $ext=null, $class=null) {
		...
		$url = ($taction) ? seturl('t='.$taction.'&xlx=',null,null,null,null) : 
		                    seturl('t=cpsavexmlgrp&xlx=',null,null,null,null);
