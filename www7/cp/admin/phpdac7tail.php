...
//include('AnsiToHtmlConverter');

class PHPTail {
	
	private $converter; /***** ansi converter **/
    ...
    public function __construct($log, $defaultUpdateTime = 2000, $maxSizeToLoad = 2097152) {
		...
		/*************************************************************************/
		//convert ansi to html colors
		//$this->converter = new AnsiToHtmlConverter();//'SolarizedXTermTheme');		
    }
    /**
    ...
    <div class="navbar navbar-default navbar-fixed-top" role="navigation">
        <div class="container">
            <div class="navbar-header">
                <a class="navbar-brand" href="#">PHPDAC7 Tail</a>
            </div>
...
$tail = new PHPTail(array(
    "Access_Log" => "/var/stereobit/phpdac7/screenlog.0",
    "Error_Log" => "/var/stereobit/phpdac7/screenlog.1",
));
*/
$screenarray = array();
for ($i=0;$i<10;$i++) {
	$sf = "/var/stereobit/phpdac7/screenlog." . strval($i);
	if (is_readable($sf))
		$screenarray['Screen-' . strval($i)] = $sf;
}
if (empty($screenarray)) {
    echo "Phpdac7 engine undefined!";
    die();	
}	
$tail = new PHPTail($screenarray);
