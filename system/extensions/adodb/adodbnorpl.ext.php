...
*/
	...
	class ADODB_Cache_File {
		...
		function __construct() {
			...
			if (empty($ADODB_INCLUDED_CSV)) {
				...
				require_once(_r('adodb/adodb-csvlib.inc.php'));
			}
		}
	...
	function _CreateCache() {
		...
		if ($this->memCache) {
			...
			if (empty($ADODB_INCLUDED_MEMCACHE)) {
				...
				require_once(_r('adodb/adodb-memcache.lib.inc.php'));
			}
	...
	function Execute($sql,$inputarr=false) {
		...
		if (($inputarr !== false) && (!is_numeric($inputarr))) { // 1,2 execute param (compatibility issue)
			...
			if (($sql) && (stristr($sql, 'select ')==false) && (stristr($sql, 'show ')==false)) {
				...
				$exTables = array('mailqueue', 'panalyze', 'pphotos', 'stats', 'syncsql', 'cpmessages', 
								  'crontab', 'cronjob', 'fsbaseline', 'fshistory', 'fsscanned', 'fsbackup',
								  'etlproducts','difproducts');//,'difprodlogs'); price,qty stats
	...
	function _Execute($sql,$inputarr=false) {
		...
		if ($this->debug) {
			...
			if (empty($ADODB_INCLUDED_LIB)) {
				...
				require_once(_r('adodb/adodb-lib.inc.php'));
			}
	...
	*/
	...
	function MetaError($err=false) {
		...
		require_once(_r('adodb/adodb-error.inc.php'));
		if ($err === false) {
			$err = $this->ErrorNo();
		}
	...
	*/
	...
	function LogSQL($enable=true) {
		...
		require_once(_r('adodb/adodb-perf.inc.php'));
	...
	*/
			...
			$relations=array())
	{
		...
		if (!isset($_ADODB_ACTIVE_DBS)) {
			...
			require_once(_r('adodb/adodb-active-record.inc.php'));
		}
...
*/
	...
	*/
	...
	if (!defined('ADODB_DATE_VERSION')) {
		...
		require_once(_r('adodb/adodb-time.inc.php'));
	}
	...
	function __toString() {
		...
		require_once(_r('adodb/toexport.inc.php'));
	...
	*/
	...
	function ADOLoadCode($dbType) {
		...
		//@include_once($file);
		@require_once(_r('adodb/drivers/adodb-'.$db.'.inc.php'));
		$ADODB_LASTDB = $class;
	...
	function NewPerfMonitor(&$conn) {
		...
		@require_once(_r('adodb/perf/perf-'.$drivername.'.inc.php'));
	...
	function NewDataDictionary(&$conn,$drivername=false) {
		...
		require_once(_r('adodb/adodb-datadict.inc.php'));
