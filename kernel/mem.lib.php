...
{	
	...
	{	
		...
		$this->extra_space = _MEMEXTRSPC; //1024 * 10; //kb //1000;// per file (shmid res inc+)
		$this->dataspace = _MEMDATASPC; //1024000 * 9; //mb //90000; //sum of shmem without preinsert		
	...
	private function _uMonitor($uuid=null, $new=false)
	{
		...
		{	
			//new monitor 	
			$port = $this->env->umon->go($uuid);
			if (!is_numeric($port))
				$this->env->cnf->_say("uMonitor ($uuid):" . $port, 'TYPE_LION');
