...
class daemon {
        ...
        function __construct($type='standalone', $prompt=false, &$env=null) {
		    ...
		    if (!extension_loaded('sockets')) 
				...
				dl('php_sockets.dll') : dl('php_sockets.so');			
        ...
        function verbose ($level, $msg) {
            ...
            if (VERBOSE && $level <= $vl && SERVER_TYPE != 'inetd') {
                  //echo str_repeat ("*", $level) . " $msg " . str_repeat ("*", $level) . PHP_EOL;
				  $this->env->_say(str_repeat ("*", $level) . " $msg " . str_repeat ("*", $level), 'TYPE_IRON');
            }
        }
        ...
        function sock_die ($msg, $return_code, $to_be_closed) {
                ...
                $this->env->_say("$msg: " . socket_strerror ($return_code), 'TYPE_LION');
...
//              fputs ($this->stdout, $string);
                if (SERVER_TYPE == 'inetd') {
					//echo $string;
					//if (isset($string))
                        $this->env->_say($string, 'TYPE_ZION', false);					
                } 
				else {
