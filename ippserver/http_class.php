...
class http_class
{
 ...
 {
  //REQUEST
  self::write2disk('ippclient.log',$string);
 ...
 {
  //RESPONSE
  self::write2disk('ippclient.log',"\r\nRESPONSE[".$this->host.':'.$this->port.']:'."\r\n");
  
  if (!$this->connected)
  {
   //RESPONSE
   self::write2disk('ippclient.log',"not connected\r\n");   
  ...
  while (!feof ($this->connection))
  {
   ...
   //RESPONSE
   self::write2disk('ippclient.log',$line);   
 ...
 private function _ReadStream ()
 {
  if (! array_key_exists ("content-length", $this->reply_headers))
  {
   ...
   //RESPONSE
   self::write2disk('ippclient.log',$this->reply_body);   
   return true;
  }
